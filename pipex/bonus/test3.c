...
void handle_here_doc(int argc, char **argv, char **envp) {
	...
	int fd[2];
	if (pipe(fd) < 0) print_error("Error: Problem occurred with pipe\n");
	if (fork() == 0) here_doc_child(fd, argv[2]);
	else {
		close(fd[1]);
...
void handle_commands(int argc, char **argv, char **envp) {
	...
	if (filein < 0 || fileout < 0) print_error("Error: Unable to open file\n");
...
void exec_commands(int start, int argc, char **argv, char **envp, int fileout) {
	...
	while (i < argc - 2) {
		if (pipe(fd) < 0) print_error("Error: Problem occurred with pipe\n");
		if ((pid = fork()) == 0) exec_child(fd, argv[i], envp);
