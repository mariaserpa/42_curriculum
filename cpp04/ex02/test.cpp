...
void testAnimal()
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		{
			animals[i] = new Dog();
			static_cast<Dog*>(animals[i])->getBrain()->setIdea(0, "I want to eat a bone");
			std::cout << "Dog idea 0: " << static_cast<Dog*>(animals[i])->getBrain()->getIdea(0) << std::endl;
		}
		else
		{
			animals[i] = new Cat();
			static_cast<Cat*>(animals[i])->getBrain()->setIdea(0, "I want to eat a tuna bowl");
			std::cout << "Cat idea 0: " << static_cast<Cat*>(animals[i])->getBrain()->getIdea(0) << std::endl;
		}
		animals[i]->makeSound();
