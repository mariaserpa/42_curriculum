...
/*   Updated: 2024/04/06 23:57:18 by mrabelo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_up(t_vars*vars)
{
	if (check_boundaries(vars, vars->player.pos_y - 1, vars->player.pos_x))
	{
		first_update(vars);
		if (vars->map.map[vars->player.pos_y][vars->player.pos_x] != EXIT)
			vars->map.map[vars->player.pos_y][vars->player.pos_x] = FLOOR;
		vars->map.display.pos_y -= SIZE;
		vars->player.pos_y -= 1;
		vars->qt_moves++;
		mlx_put_image_to_window(vars->mlx, vars->mlx_win, vars->img.player_up, \
						vars->map.display.pos_x, vars->map.display.pos_y);
		if (vars->map.map[vars->player.pos_y][vars->player.pos_x] == FLOOR)
			vars->map.map[vars->player.pos_y][vars->player.pos_x] = START;
		if (vars->map.map[vars->player.pos_y][vars->player.pos_x] == COLLECT)
		{
			vars->map.map[vars->player.pos_y][vars->player.pos_x] = FLOOR;
			vars->qt_collected++;
		}
